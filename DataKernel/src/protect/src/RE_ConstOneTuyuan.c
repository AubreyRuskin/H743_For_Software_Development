...
/*       文件名                                           版本                 */
/*                                                                              */
/*      RE_ConstOneTuyuan.c                                    1.0                   */
/*                                                                              */
/*                                                                               */
/*     文件描述                                                                  */
/*                                                                              */
/*       该文件定义了保护功能模块中的恒1元件的实现代码                        */
/*                                                                              */
/*      作者                                                                      */
/*                                                                              */
/*      张云                                                                  */
...
EP_STATUS   RE_ConstOneTuyuanReadFileInit(FILE  *fp,uint32_t  ulReadOffsetToBegain,
        TUYUAN_READ_FILE_INIT_DATA   *pTuyuanInitData,
        NODE ** pRtElemInitNodePointer)
{

    NODE  *pNode;
    ConstOne_Init_Node_Type    *pInitNode;
    BOOL   bReadSuccess;
    int  i;
    ...
    if(pNode==NULL)
    {
        LOG_Dbg_Msg("ConstOne  Tuyuan  malloc  failiure!\n",0,0,0,0,0,0);
        return  EP_BUF_ERR;

    }

    pInitNode=(ConstOne_Init_Node_Type    *)malloc(sizeof(ConstOne_Init_Node_Type));
    if(pInitNode==NULL)
    {
        LOG_Dbg_Msg("ConstOne  Tuyuan  malloc  failiure!\n",0,0,0,0,0,0);
        return  EP_BUF_ERR;
    ...
    /* 进行节点其他未获得初值的信息,赋初值 */
    bOtherInitSuccess=RE_ConstOneTuyuanReadFileOtherInit
                      (pInitNode);
...
EP_STATUS   RE_ConstOneTuyuanScanInit(NODE * pElemInitNode,NODE  *pElemScanNode,
...
/*  在读取文件后,进行图元相关未能从文件中获得的其他信息的初始化
      ...
      返回值,成功,返回真,失败返回假
*/
BOOL      RE_ConstOneTuyuanReadFileOtherInit
(ConstOne_Init_Node_Type * pElemInitNodePointer)
{

    /*  设置图元类型*/
    pElemInitNodePointer->PublicElemData.elem.ucType=RE_ONE;
    /* 设置包裹后的初始化函数 */
    pElemInitNodePointer->PublicElemData.pfInitScanFunc=
        RE_ConstOneTuyuanScanInit;

    /* 设置获得IO输出的函数 */
    pElemInitNodePointer->PublicElemData.pfGetScanNodeOutIOFunc
        =RE_ConstOneTuyuanGetOutIO;
    return   TRUE;
...
EP_STATUS   RE_ConstOneCreateScanNodeInit(
    NODE **pReturnElemScanNodePointer,
    NODE * pElemInitNode)
{
    NODE  *pNode;
    ConstOne_Scan_Node_Type    *pScanNode;
    ConstOne_Init_Node_Type    *pInitNode;

    pInitNode=(ConstOne_Init_Node_Type    *)pElemInitNode->pTuyuan;
    ...
    pScanNode=(ConstOne_Scan_Node_Type    *)malloc
              (sizeof(ConstOne_Scan_Node_Type));
    if(pScanNode==NULL)
    {
        LOG_Dbg_Msg("ConstOne  Tuyuan  malloc  failiure!\n",0,0,0,0,0,0);
        return  EP_BUF_ERR;
    }
    pNode->ulTuyuanType=RE_ONE_SCAN;
...
EP_ELEM_IO *  RE_ConstOneTuyuanGetOutIO
(uint16_t   unOutNum,NODE  * pScanNode)
{
    ConstOne_Scan_Node_Type    *pTuyuanNode;

    pTuyuanNode=(ConstOne_Scan_Node_Type    *)pScanNode->pTuyuan;
    if(unOutNum>=1)
    {
        /* 若越界，则出错 */

        LOG_Dbg_Msg("Get One ConstOne  Tuyuan  OutputIO  Beyond  Bound!\n",0,0,0,0,0,0);
        assert(FALSE);
        return  NULL;
