...
/*       文件名                                           版本                 */
/*                                                                              */
/*      RE_ImportTuyuan.c                                    1.0                   */
/*                                                                              */
/*                                                                               */
/*     文件描述                                                                  */
/*                                                                              */
/*       该文件定义了保护功能模块中的端口引入元件的代码文件                        */
/*                                                                              */
/*      作者                                                                      */
/*                                                                              */
/*      张云                                                                  */
...
EP_STATUS   RE_ImportTuyuanReadFileInit(FILE  *fp,uint32_t  ulReadOffsetToBegain,
                                        TUYUAN_READ_FILE_INIT_DATA   *pTuyuanInitData,
                                        NODE ** pRtElemInitNodePointer)
{

    NODE  *pNode;
    Import_Init_Node_Type    *pInitNode;
    BOOL   bReadSuccess;
    ...
    if(pNode==NULL)
    {
        LOG_Dbg_Msg("Import  Tuyuan  malloc  failiure!\n",0,0,0,0,0,0);
        return  EP_BUF_ERR;

    }
    pInitNode=(Import_Init_Node_Type    *)malloc(sizeof(Import_Init_Node_Type));
    if(pInitNode==NULL)
    {
        LOG_Dbg_Msg("Imort  Tuyuan  malloc  failiure!\n",0,0,0,0,0,0);
        return  EP_BUF_ERR;

    }
    /* 将节点的信息指针给节点 */
    pNode->ulTuyuanType=RE_IMPORT;
    ...
    for(i=0; i<1; i++)
    {
        ...
        bReadSuccess=
            RE_ImportTuyuanGetExternImportSourceFileReadInit(
                fp,pInitNode);
        if(!bReadSuccess)
        {
            return   EP_SYS_ERR;
    ...
    /* 进行节点其他未获得初值的信息,赋初值 */
    bOtherInitSuccess=RE_ImportTuyuanReadFileOtherInit
                      (pInitNode);
...
EP_STATUS   RE_ImportTuyuanScanInit(NODE * pElemInitNode,NODE  *pElemScanNode,
...
/*  在读取文件后,进行图元相关未能从文件中获得的其他信息的初始化
      ...
      返回值,成功,返回真,失败返回假
*/
BOOL      RE_ImportTuyuanReadFileOtherInit
(Import_Init_Node_Type * pElemInitNodePointer)
{
    pElemInitNodePointer->PublicElemData.elem.ucType=RE_IMPORT;
    /* 设置包裹后的初始化函数 */
    pElemInitNodePointer->PublicElemData.pfInitScanFunc=
        RE_ImportTuyuanScanInit;

    /* 设置获得IO输出的函数 */
    pElemInitNodePointer->PublicElemData.pfGetScanNodeOutIOFunc
        =RE_ImportTuyuanGetOutIO;
...
/*　当外部输入来源为端口引入时,读取名称,并进行相应的初始化
    参数   fp  ,文件当前指针
           pElemInitNode,初始化节点指针
*/
BOOL   RE_ImportTuyuanGetExternImportSourceFileReadInit(
    FILE  *fp,
    Import_Init_Node_Type *  pElemInitNode
)
{
...
void   RE_ImportTuyuanSetExternImportOutputPt
(EP_ELEM_IO  *pIO,ExternImportOuterInput_Scan_Node_Type
 *pScanNode)
{
    pScanNode->pIoOut=pIO;
    pScanNode->ioOut=*(pScanNode->pIoOut);/*  */
...
EP_STATUS   RE_ImportCreateScanNodeInit(
    NODE **pReturnElemScanNodePointer,
    NODE * pElemInitNode)
{
    NODE  *pNode;
    Import_Init_Node_Type    *pInitNode;
    ExternImportOuterInput_Scan_Node_Type    *pScanNode;
    pInitNode=(Import_Init_Node_Type    *)pElemInitNode->pTuyuan;
    ...
    pScanNode=
        (ExternImportOuterInput_Scan_Node_Type   *)
        malloc
        (sizeof(ExternImportOuterInput_Scan_Node_Type));
    if(pScanNode==NULL)
    {
        LOG_Dbg_Msg("Import  Tuyuan  malloc  failiure!\n",0,0,0,0,0,0);
        return  EP_BUF_ERR;
    }
    pNode->ulTuyuanType=RE_EXTERN_IMPORT_OUTERINPUT_SCAN;
    pNode->pTuyuan=(void  *)pScanNode;
    pScanNode->ucSignalType=pInitNode->
...
EP_ELEM_IO *  RE_ImportTuyuanGetOutIO
(uint16_t   unOutNum,NODE  * pScanNode)
{

    ExternImportOuterInput_Scan_Node_Type    *pTuyuanNode;
    pTuyuanNode=
        (ExternImportOuterInput_Scan_Node_Type    *)pScanNode->pTuyuan;
    if(unOutNum>=1)
    {
        /* 若越界，则出错 */

        LOG_Dbg_Msg("Get Import  Tuyuan  OutputIO  Beyond  Bound!\n",0,0,0,0,0,0);
        assert(FALSE);
        return  NULL;
...
/* 2011-7-27  ZY
   设置端口引入图元的集中方式的输出指针
     ...
     返回   无
*/
void   RE_ImportTuyuanSetCollectOutputPt
(void  *pIO,ExternImportOuterInput_Scan_Node_Type
 *pScanNode)
{
    pScanNode->pCollectInput=pIO;
    if(pScanNode->pCollectOut)
    {
        /* 注意这里要加判定 */
        *(pScanNode->pCollectOut)=*(pScanNode->pIoOut);/*注意,要在这里赋初值  */
    }
}
