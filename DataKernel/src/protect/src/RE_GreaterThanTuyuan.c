...
/*       文件名                                           版本                 */
/*                                                                              */
/*      RE_GreaterThanTuyuan.c                                    1.0                   */
/*                                                                              */
/*                                                                               */
/*     文件描述                                                                  */
/*                                                                              */
/*       该文件定义了保护功能模块中的大于比较元件的实现代码                        */
/*                                                                              */
/*      作者                                                                      */
/*                                                                              */
/*      张云                                                                  */
...
EP_STATUS   RE_GreaterThanTuyuanReadFileInit(FILE  *fp,uint32_t  ulReadOffsetToBegain,
        TUYUAN_READ_FILE_INIT_DATA   *pTuyuanInitData,
        NODE ** pRtElemInitNodePointer)
{
    NODE   *pNode;
    GreaterThan_Init_Node_Type    *pInitNode;
    BOOL   bReadSuccess;
    int  i;
    ...
    if(pNode==NULL)
    {
        LOG_Dbg_Msg("Greater  Than  Tuyuan  malloc  failiure!\n",0,0,0,0,0,0);
        return  EP_BUF_ERR;

    }
    pInitNode=(GreaterThan_Init_Node_Type    *)malloc(sizeof(GreaterThan_Init_Node_Type));
    if(pInitNode==NULL)
    {
        LOG_Dbg_Msg("Greater  Than  Tuyuan  malloc  failiure!\n",0,0,0,0,0,0);
        return  EP_BUF_ERR;
    ...
    /* 将节点的信息指针给节点 */
    pNode->ulTuyuanType=RE_GREATERTHAN;
    ...
    bSignalInitSuccess=RE_GreaterThanTuyuanInputSignalInit(
    ...
    for(i=0; i<1; i++)
    {
        ...
        else
        {
            ...
            LOG_Dbg_Msg("Read  Greater Than   Lubo  Flag   from  Logrp  SeqFile  error!\n",0,0,0,0,0,0);
            return  EP_SYS_ERR;
        }
        ...
        else
        {
            ...
            LOG_Dbg_Msg("Read  Greater  Than  FlagSet  Flag   from  Logrp  SeqFile  error!\n",0,0,0,0,0,0);
            return  EP_SYS_ERR;
        }
        ...
        {
            LOG_Dbg_Msg("Read  Greater  Than  Yaoxin  Flag   from  Logrp  SeqFile  error!\n",0,0,0,0,0,0);
            return  EP_SYS_ERR;
        }
    ...
    /* 进行节点其他未获得初值的信息,赋初值 */
    bOtherInitSuccess=RE_GreaterThanTuyuanReadFileOtherInit
                      (pInitNode);
...
EP_STATUS   RE_GreaterThanTuyuanScanInit(NODE * pElemInitNode,NODE  *pElemScanNode,
        LIST  *pGrpScanNodeList,BOOL  bPartGrpRunFlag)
{

    GreaterThan_Init_Node_Type    *pInitNode;
    GreaterThan_Scan_Node_Type    *pScanNode;
    NODE  *  pCurSourceNode;
    int  i;
    ...
    pInitNode=(GreaterThan_Init_Node_Type    *)pElemInitNode->pTuyuan;
    pScanNode=(GreaterThan_Scan_Node_Type    *)pElemScanNode->pTuyuan;
    ...
    for(i=0; i<(pInitNode->PublicElemData.elem.unInNum); i++)
    {
        ...
        {
            LOG_Dbg_Msg("error,Get  GreaterThan  Tuyuan  Input Source  Init  failure!\n",0,0,0,0,0,0);
        ...
        if(pInElem==NULL)
        {
            ...
            LOG_Dbg_Msg("error,Get  GreaterThan  Tuyuan  input  info  failure!\n",0,0,0,0,0,0);
        ...
        SetCurOutAttribResult=RE_GreaterThanTuyuanSetInputIO
                              (i,pInElem,pElemScanNode);
        if(SetCurOutAttribResult!=EP_SUCCESS)
        {

            LOG_Dbg_Msg("error,Set  GreaterThan  Tuyuan  input  info  failure!\n",0,0,0,0,0,0);
        ...
        {
            LOG_Dbg_Msg("error,GreaterThan Tuyuan One  Input  Signal Type  can't  Match!\n",0,0,0,0,0,0);
    ...
    /* 循环设定图元的输出属性*/
    pCurElemOutput=RE_GreaterThanTuyuanGetOutIO
                   (0,pElemScanNode);
    /*   */
    if(pCurElemOutput==NULL)
    {

        LOG_Dbg_Msg("error,Can't  Get GreaterThan  Tuyuan One Output  info!\n",0,0,0,0,0,0);
    ...
    if(bPartGrpRunFlag)
    {
        ...
        if((*pbCurLuboSetFlag))
        {
            ...
            if(SetCurOutAttribResult!=EP_SUCCESS)
            {
                LOG_Dbg_Msg("Set  GreaterThan  Tuyuan  \'%s\'  Lubo  Value  Error!\n"
                ...
                else if(ENG_MODE == 1)
                {
                    ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                               ER_REPORT|ER_ALARM|ER_LOCK,
                               "logic grp rslv err:set great  element disturbance record id \'%s\' err\n"
                               ,(int)(*pstrCurLuboID),0);
                }
                assert(FALSE);
        ...
        if((*pbCurFlagSetFlag))
        {
            ...
            if(SetCurOutAttribResult!=EP_SUCCESS)
            {
                LOG_Dbg_Msg("Set  GreaterThan  Tuyuan  \'%s\'  Flag  Value  Error!\n"
                            ,(int)(*pstrCurFlagID),0,0,0,0,0);
                if(ENG_MODE == 0)
                {
                    ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                               ER_REPORT|ER_ALARM|ER_LOCK,
                               "逻辑图解析错误:设置大于比较图元标志错误,标志标识为 \'%s\'\n"
                               ,(int)(*pstrCurFlagID),0);
                }
                else if(ENG_MODE == 1)
                {
                    ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                               ER_REPORT|ER_ALARM|ER_LOCK,
                               "logic grp rslv err:set great  element flag id \'%s\' err\n"
                               ,(int)(*pstrCurFlagID),0);
                }
                assert(FALSE);
                return   SetCurOutAttribResult;
        ...
        if((*pbCurYaoxinSetFlag))
        {
            ...
            if(SetCurOutAttribResult!=EP_SUCCESS)
            {
                LOG_Dbg_Msg("Set  GreaterThan  Tuyuan  \'%s\'  Yaoxin  Value  Error!\n"
                ...
                else if(ENG_MODE == 1)
                {
                    ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                               ER_REPORT|ER_ALARM|ER_LOCK,
                               "logic grp rslv err:set great  element romete signal id \'%s\' err\n"
                               ,(int)(*pstrCurYaoxinID),0);
                }
                assert(FALSE);
                return   SetCurOutAttribResult;
...
/*  在读取文件后,进行图元相关未能从文件中获得的其他信息的初始化
      ...
      返回值,成功,返回真,失败返回假
*/
BOOL      RE_GreaterThanTuyuanReadFileOtherInit
(GreaterThan_Init_Node_Type * pElemInitNodePointer)
{

    pElemInitNodePointer->PublicElemData.elem.ucType=RE_GREATERTHAN;
    /* 设置包裹后的初始化函数 */
    pElemInitNodePointer->PublicElemData.pfInitScanFunc=
        RE_GreaterThanTuyuanScanInit;

    /* 设置获得IO输出的函数 */
    pElemInitNodePointer->PublicElemData.pfGetScanNodeOutIOFunc
        =RE_GreaterThanTuyuanGetOutIO;
 ...
 */
BOOL     RE_GreaterThanTuyuanInputSignalInit(uint8_t  ucSignalType,
        GreaterThan_Init_Node_Type    *pElemInitNodePointer)
{
    switch(ucSignalType)
    {
        ...
        case OHM_PER_METER:
            ...
            break;
        default:
            LOG_Dbg_Msg("Error,GreaterThan  Tuyuan  Input  Signal   Type  isn't  Expected!\n",0,0,0,0,0,0);
            assert(FALSE);  /* 若非以上类型,则告警   */
            return  FALSE;
            break;
    }
...
EP_STATUS   RE_GreaterThanCreateScanNodeInit(
    ...
    NODE * pElemInitNode)
{
    ...
    pScanNode=(GreaterThan_Scan_Node_Type    *)malloc
              (sizeof(GreaterThan_Scan_Node_Type));
    if(pScanNode==NULL)
    {
        LOG_Dbg_Msg("GreaterThan  Tuyuan  malloc  failiure!\n",0,0,0,0,0,0);
        return  EP_BUF_ERR;
    }
    /*  设置图元类型*/
    ...
    switch(pInitNode->ucSignalDataType)
    {
        case  0:
            ...
            pNode->ulTuyuanType=RE_FLOAT_GREATERTHAN_SCAN;
            break;
        case  1:

            pNode->ulTuyuanType=RE_UNSIGNED_32INT_GREATERTHAN_SCAN;

            break;
        case  2:

            pNode->ulTuyuanType=RE_SIGNED_32INT_GREATERTHAN_SCAN;
...
EP_ELEM_IO *  RE_GreaterThanTuyuanGetOutIO
(uint16_t   unOutNum,NODE  * pScanNode)
{

    GreaterThan_Scan_Node_Type    *pTuyuanNode;

    pTuyuanNode=(GreaterThan_Scan_Node_Type    *)pScanNode->pTuyuan;
    if(unOutNum>=1)
    {
        /* 若越界，则出错 */

        LOG_Dbg_Msg("Get One  GreaterThan  Tuyuan  OutputIO  Beyond  Bound!\n",0,0,0,0,0,0);
        assert(FALSE);
        return  NULL;
...
EP_STATUS   RE_GreaterThanTuyuanSetInputIO
 ...
 NODE  * pScanNode)
{
    ...
    switch(ucInputNum)
    {
        ...
        default:
            LOG_Dbg_Msg("Set One GreaterThan  Tuyuan  InputIO  Beyond  Bound!\n",0,0,0,0,0,0);
            assert(FALSE);
            return  EP_SYS_ERR;
            break;
    }
