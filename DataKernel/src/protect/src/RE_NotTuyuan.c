...
EP_STATUS   RE_NotTuyuanReadFileInit(FILE  *fp,uint32_t  ulReadOffsetToBegain,
                                     TUYUAN_READ_FILE_INIT_DATA   *pTuyuanInitData,
                                     NODE ** pRtElemInitNodePointer)
{
    NODE  *pNode;
    Not_Init_Node_Type    *pInitNode;
    BOOL   bReadSuccess;
    int  i;
    ...
    if(pNode==NULL)
    {
        LOG_Dbg_Msg("Not  Tuyuan  malloc  failiure!\n",0,0,0,0,0,0);
        return  EP_BUF_ERR;

    }

    pInitNode=(Not_Init_Node_Type    *)malloc(sizeof(Not_Init_Node_Type));
    if(pInitNode==NULL)
    {
        LOG_Dbg_Msg("Not  Tuyuan  malloc  failiure!\n",0,0,0,0,0,0);
        return  EP_BUF_ERR;
    ...
    else
    {
        ...
        LOG_Dbg_Msg("Read Not  Tuyuan  Lubo  Flag   from  Logrp  SeqFile  error!\n",0,0,0,0,0,0);
        return  EP_SYS_ERR;
    }
    ...
    else
    {
        ...
        LOG_Dbg_Msg("Read  Not Tuyuan  FlagSet  Flag   from  Logrp  SeqFile  error!\n",0,0,0,0,0,0);
        return  EP_SYS_ERR;
    }
    ...
    {
        LOG_Dbg_Msg("Read  Not  Tuyuan   Yaoxin  Flag   from  Logrp  SeqFile  error!\n",0,0,0,0,0,0);
        return  EP_SYS_ERR;
    }
    ...
    /* 进行节点其他未获得初值的信息,赋初值 */
    bOtherInitSuccess=RE_NotTuyuanReadFileOtherInit
                      (pInitNode);
...
EP_STATUS   RE_NotTuyuanScanInit(NODE * pElemInitNode,NODE  *pElemScanNode,
                                 LIST  *pGrpScanNodeList,BOOL  bPartGrpRunFlag)
{
    ...
    pInitNode=(Not_Init_Node_Type    *)pElemInitNode->pTuyuan;
    ...
    {
        LOG_Dbg_Msg("error,Get  Not  Tuyuan  Input Source  Init  failure!\n",0,0,0,0,0,0);
    ...
    if(pInElem==NULL)
    {
        ...
        LOG_Dbg_Msg("error,Get  Not  Tuyuan  input  info  failure!\n",0,0,0,0,0,0);
    ...
    SetCurOutAttribResult=RE_NotTuyuanSetInputIO
                          (0,pInElem,pElemScanNode);
    if(SetCurOutAttribResult!=EP_SUCCESS)
    {

        LOG_Dbg_Msg("error,Set  Not  Tuyuan  input  info  failure!\n",0,0,0,0,0,0);
    ...
    {
        LOG_Dbg_Msg("error,Not Tuyuan One  Input  Signal Type  can't  Match!\n",0,0,0,0,0,0);
    ...
    pCurElemOutput=RE_NotTuyuanGetOutIO
                   (0,pElemScanNode);

    if(pCurElemOutput==NULL)
    {

        LOG_Dbg_Msg("error,Can't  Get  Not  Tuyuan One Output  info!\n",0,0,0,0,0,0);
    ...
    if(bPartGrpRunFlag)
    {
        ...
        if((*pbCurLuboSetFlag))
        {
            ...
            if(SetCurOutAttribResult!=EP_SUCCESS)
            {
                ...
                else if(ENG_MODE == 1)
                {
                    ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                               ER_REPORT|ER_ALARM|ER_LOCK,
                               "logic grp rslv err:set not   element  disturbance record id \'%s\' err\n"
                               ,(int)(*pstrCurLuboID),0);
                }
        ...
        if((*pbCurFlagSetFlag))
        {
            ...
            if(SetCurOutAttribResult!=EP_SUCCESS)
            {
                LOG_Dbg_Msg("Set  Not  Tuyuan  \'%s\'  Flag  Value  Error!\n"
                ...
                else if(ENG_MODE == 1)
                {
                    ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                               ER_REPORT|ER_ALARM|ER_LOCK,
                               "logic grp rslv err:set not  element flag id \'%s\' err\n"
                               ,(int)(*pstrCurFlagID),0);
                }
        ...
        if((*pbCurYaoxinSetFlag))
        {
            ...
            if(SetCurOutAttribResult!=EP_SUCCESS)
            {
                LOG_Dbg_Msg("Set  Not  Tuyuan  \'%s\'  Yaoxin  Value  Error!\n"
                            ,(int)(*pstrCurYaoxinID),0,0,0,0,0);
                if(ENG_MODE == 0)
                {
                    ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                               ER_REPORT|ER_ALARM|ER_LOCK,
                               "逻辑图解析错误:设置非门图元遥信错误,遥信标识为\'%s\'\n"
                ...
                else if(ENG_MODE == 1)
                {
                    ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                               ER_REPORT|ER_ALARM|ER_LOCK,
                               "logic grp rslv err:set not  element remote signal id \'%s\' err\n"
                               ,(int)(*pstrCurYaoxinID),0);
                }
                assert(FALSE);  /* 若非以上类型,则告警   */
                return   SetCurOutAttribResult;
...
/*  在读取文件后,进行图元相关未能从文件中获得的其他信息的初始化
      ...
      返回值,成功,返回真,失败返回假
*/
BOOL      RE_NotTuyuanReadFileOtherInit
(Not_Init_Node_Type * pElemInitNodePointer)
{

    pElemInitNodePointer->PublicElemData.elem.ucType=RE_NOT;
    /*  设置图元输入数组指针 */
    /* 设置包裹后的初始化函数 */
    pElemInitNodePointer->PublicElemData.pfInitScanFunc=
        RE_NotTuyuanScanInit;
    /* 设置获得IO输出的函数 */
    pElemInitNodePointer->PublicElemData.pfGetScanNodeOutIOFunc
        =RE_NotTuyuanGetOutIO;
...
EP_STATUS   RE_NotCreateScanNodeInit(
    NODE **pReturnElemScanNodePointer,
    NODE * pElemInitNode)
{
    NODE  *pNode;
    Not_Scan_Node_Type    *pScanNode;
    Not_Init_Node_Type    *pInitNode;
    pInitNode=(Not_Init_Node_Type    *)pElemInitNode->pTuyuan;
    ...
    pScanNode=(Not_Scan_Node_Type    *)malloc
              (sizeof(Not_Scan_Node_Type));
    if(pScanNode==NULL)
    {
        LOG_Dbg_Msg("Not  Tuyuan  malloc  failiure!\n",0,0,0,0,0,0);
        return  EP_BUF_ERR;
    }
    pNode->ulTuyuanType=RE_NOT_SCAN;
...
EP_ELEM_IO *  RE_NotTuyuanGetOutIO
(uint16_t   unOutNum,NODE  * pScanNode)
{
    Not_Scan_Node_Type    *pTuyuanNode;

    pTuyuanNode=(Not_Scan_Node_Type    *)pScanNode->pTuyuan;
    if(unOutNum>=1)
    {
        /* 若越界，则出错 */

        LOG_Dbg_Msg("Get One Not  Tuyuan  OutputIO  Beyond  Bound!\n",0,0,0,0,0,0);
        assert(FALSE);
        return  NULL;
...
EP_STATUS   RE_NotTuyuanSetInputIO
 ...
 NODE  * pScanNode)
{
    ...
    if(ucInputNum>=1)
    {
        ...
        LOG_Dbg_Msg("Set One  Not Tuyuan  InputIO  Beyond  Bound!\n",0,0,0,0,0,0);
        assert(FALSE);
        return  EP_SYS_ERR;
