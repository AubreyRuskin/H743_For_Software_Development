...
/*       文件名                                           版本                 */
/*                                                                              */
/*      RE_RelayStartTuyuan.C                             1.0                   */
/*                                                                              */
/*                                                                               */
/*     文件描述                                                                  */
/*                                                                              */
/*       该文件定义了保护功能模块中的保护启停元件的代码实现                       */
/*                                                                              */
/*      作者                                                                      */
/*                                                                              */
/*      张云                                                                  */
...
EP_STATUS   RE_RelayStartTuyuanReadFileInit(FILE  *fp,uint32_t  ulReadOffsetToBegain,
        TUYUAN_READ_FILE_INIT_DATA   *pTuyuanInitData,
        NODE ** pRtElemInitNodePointer)
{
    NODE  *pNode;
    RelayStart_Init_Node_Type    *pInitNode;
    BOOL   bReadSuccess;
    int  i;
    ...
    if(pNode==NULL)
    {
        LOG_Dbg_Msg("RelayStart  Tuyuan  malloc  failiure!\n",0,0,0,0,0,0);
        return  EP_BUF_ERR;

    }
    pInitNode=(RelayStart_Init_Node_Type    *)malloc(sizeof(RelayStart_Init_Node_Type));
    if(pInitNode==NULL)
    {
        LOG_Dbg_Msg("RelayStart  Tuyuan  malloc  failiure!\n",0,0,0,0,0,0);
        return  EP_BUF_ERR;
    ...
    /* 将节点的信息指针给节点 */
    pNode->ulTuyuanType=RE_RELAYSTART;
    ...
    /* 进行节点其他未获得初值的信息,赋初值 */
    bOtherInitSuccess=RE_RelayStartTuyuanReadFileOtherInit
                      (pInitNode);
...
EP_STATUS   RE_RelayStartTuyuanScanInit(NODE * pElemInitNode,NODE  *pElemScanNode,
                                        LIST  *pGrpScanNodeList,BOOL  bPartGrpRunFlag)
{
    ...
    EP_ELEM_IO *  pInElem;
    TripEnableOuterOutput_Scan_Node_Type  *pTripEnableScanNode;
    EP_STATUS  SetCurOutAttribResult;
    uint8_t   ucCurInSignalType;

    pInitNode=(RelayStart_Init_Node_Type    *)pElemInitNode->pTuyuan;
    pucCurInSignalType=pInitNode->aucSourceSignalTypeArr;
    ...
    for(i=0; i<(pInitNode->PublicElemData.elem.unInNum); i++)
    {
        ...
        {
            LOG_Dbg_Msg("error,Get  RelayStart  Tuyuan  Input Source  Init  failure!\n",0,0,0,0,0,0);
        ...
        if(pInElem==NULL)
        {
            LOG_Dbg_Msg("error,Get  RelayStart  Tuyuan  input  info  failure!\n",0,0,0,0,0,0);
            assert(FALSE);  /* 若非以上类型,则告警   */
            return  EP_SYS_ERR;
        ...
        SetCurOutAttribResult=RE_RelayStartTuyuanSetInputIO
                              (i,pInElem,pElemScanNode);
        if(SetCurOutAttribResult!=EP_SUCCESS)
        {
            LOG_Dbg_Msg("error,Set  RelayStart  Tuyuan  input  info  failure!\n",0,0,0,0,0,0);
        ...
        {
            LOG_Dbg_Msg("error,RelayStart Tuyuan One  Input  Signal Type  can't  Match!\n",0,0,0,0,0,0);
    ...
    pTripEnableScanNode=(TripEnableOuterOutput_Scan_Node_Type *)
    ...
    if(pTripEnableScanNode->bDefEventToTrigger)
    {
        ...
        {
            LOG_Dbg_Msg("error,Get RelayStart Tuyuan Trigger \'%s\' EventID Info Failure!\n"
                        ,(int)(pInitNode->strTripTriggerEventID),0,0,0,0,0);

            if(ENG_MODE == 0)
            {
                ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                           ER_REPORT|ER_ALARM|ER_LOCK,"逻辑图解析错误:获得保护启停图元触发的事件\'%s\'的信息错误\n"
                           ,(int)(pInitNode->strTripTriggerEventID),0);
            }
            else if(ENG_MODE == 1)
            {
                ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                           ER_REPORT|ER_ALARM|ER_LOCK,"logic grp rslv  err:get relay start  element trigger event \'%s\' info err\n"
                           ,(int)(pInitNode->strTripTriggerEventID),0);
            }
            assert(FALSE);
...
/*  在读取文件后,进行图元相关未能从文件中获得的其他信息的初始化
      ...
      返回值,成功,返回真,失败返回假
*/
BOOL      RE_RelayStartTuyuanReadFileOtherInit
(RelayStart_Init_Node_Type * pElemInitNodePointer)
{

    int  nTriggerEventIDLen;
    pElemInitNodePointer->PublicElemData.elem.ucType=RE_RELAYSTART;
    /* 设置包裹后的初始化函数 */
    pElemInitNodePointer->PublicElemData.pfInitScanFunc=
        RE_RelayStartTuyuanScanInit;
...
EP_STATUS   RE_RelayStartCreateScanNodeInit(
    ...
    NODE * pElemInitNode)
{
    ...
    NODE  *pNode;
    RelayStart_Init_Node_Type    *pInitNode;
    TripEnableOuterOutput_Scan_Node_Type   *pScanNode;
    ...
    pScanNode=(TripEnableOuterOutput_Scan_Node_Type    *)
              malloc
              (sizeof(TripEnableOuterOutput_Scan_Node_Type));
    if(pScanNode==NULL)
    {
        LOG_Dbg_Msg("RelayStart  Tuyuan  malloc  failiure!\n",0,0,0,0,0,0);
        return  EP_BUF_ERR;
    }
    pNode->ulTuyuanType=RE_TRIP_ENABLE_OUTEROUTPUT_SCAN;
...
EP_STATUS   RE_RelayStartTuyuanSetInputIO
(uint8_t  ucInputNum,EP_ELEM_IO *  pElemIO,
 NODE  * pScanNode)
{
    TripEnableOuterOutput_Scan_Node_Type    *pTuyuanNode;
    pTuyuanNode=(TripEnableOuterOutput_Scan_Node_Type    *)
                pScanNode->pTuyuan;
    if(ucInputNum>=1)
    {
        /* 若越界，则出错 */
        LOG_Dbg_Msg("Set RelayStart Tuyuan  InputIO  Beyond  Bound!\n",0,0,0,0,0,0);
        assert(FALSE);
        return  EP_SYS_ERR;
    }
...
void     RE_TripEnableOuterOutputTuyuanScan(NODE *pElemScanNode)
{
