...
/*       文件名                                           版本                 */
/*                                                                              */
/*      RE_AmpAngTuyuan.c                                    1.0                  */
/*                                                                              */
/*                                                                               */
/*     文件描述                                                                  */
/*                                                                              */
/*       该源代码文件定义了保护功能模块中的幅值相角元件的实现                       */
/*                                                                              */
/*      作者                                                                      */
/*                                                                              */
/*      张云                                                                  */
...
EP_STATUS   RE_AmpAngTuyuanReadFileInit(FILE  *fp,uint32_t  ulReadOffsetToBegain,
                                        ...
                                        NODE ** pRtElemInitNodePointer)
{
    ...
    AmpAng_Init_Node_Type    *pInitNode;
    BOOL   bReadSuccess;
    int  i;
    /* 文件指针重新定位  */
    int  OpeSuccess;
    ...
    if(pNode==NULL)
    {
        LOG_Dbg_Msg("AmpAng  Tuyuan  malloc  failiure!\n",0,0,0,0,0,0);
        return  EP_BUF_ERR;

    }

    pInitNode=(AmpAng_Init_Node_Type    *)malloc(sizeof(AmpAng_Init_Node_Type));
    if(pInitNode==NULL)
    {
        LOG_Dbg_Msg("AmpAng  Tuyuan  malloc  failiure!\n",0,0,0,0,0,0);
        return  EP_BUF_ERR;
    ...
    for(i=0; i<1; i++)
    {
        ...
        {
            LOG_Dbg_Msg("Error, AmpAng Tuyuan Input Signal Type  is not  expected!\n"
                        ,0,0,0,0,0,0);
            if(ENG_MODE == 0)
            {
                ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                           ER_REPORT|ER_ALARM|ER_LOCK,
                           "逻辑图解析错误:读取幅值相角图元的输入信号类型不是预期类型\n"
                           ,0,0);
            }
            else if(ENG_MODE == 1)
            {
                ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                           ER_REPORT|ER_ALARM|ER_LOCK,
                           "logic grp rslv err:read AmpAng  element Input  signal type is not  expect type\n"
                           ,0,0);
            }
            assert(FALSE);
            return   EP_SYS_ERR;
    ...
    for(i=0; i<2; i++)
    {
        ...
        {
            LOG_Dbg_Msg("Error, AmpAng Tuyuan Output Signal Type  is not  expected!\n"
                        ,0,0,0,0,0,0);
            if(ENG_MODE == 0)
            {
                ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                           ER_REPORT|ER_ALARM|ER_LOCK,
                           "逻辑图解析错误:读取幅值相角图元的输出信号类型不是预期类型\n"
                           ,0,0);
            }
            else if(ENG_MODE == 1)
            {
                ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                           ER_REPORT|ER_ALARM|ER_LOCK,
                           "logic grp rslv err:read AmpAng  element Output  signal type is not  expect type\n"
                           ,0,0);
            }
            assert(FALSE);
            return   EP_SYS_ERR;
        ...
        {
            LOG_Dbg_Msg("Error, Read AmpAng Tuyuan AO Flag Error!\n",
                        0, 0, 0, 0, 0, 0);
            ...
            else if (ENG_MODE == 1)
            {
                ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                           ER_REPORT|ER_ALARM|ER_LOCK,
                           "logic grp rslv err: read AmpAng Tuyuan element AO set err\n",
                           0, 0);
            }
        ...
        {
            LOG_Dbg_Msg("Error, Read AmpAng  Tuyuan  Lubo  Flag  Error!\n"
                        ,0,0,0,0,0,0);
            if(ENG_MODE == 0)
            {
                ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                           ER_REPORT|ER_ALARM|ER_LOCK,
                           "逻辑图解析错误:读取幅值相角图元的录波设置错误\n"
                           ,0,0);
            }
            else if(ENG_MODE == 1)
            {
                ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                           ER_REPORT|ER_ALARM|ER_LOCK,
                           "logic grp rslv err:read AmpAng  element disturbance record set err\n"
                           ,0,0);
            }
            assert(FALSE);  /* 若非以上类型,则告警   */
            return  EP_SYS_ERR;
        ...
        {
            LOG_Dbg_Msg("Error, Read AmpAng  Tuyuan  FlagSet  Flag  Error!\n"
                        ,0,0,0,0,0,0);
            if(ENG_MODE == 0)
            {
                ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                           ER_REPORT|ER_ALARM|ER_LOCK,
                           "逻辑图解析错误:读取幅值相角图元的标志设置错误\n"
                           ,0,0);
            }
            else if(ENG_MODE == 1)
            {
                ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                           ER_REPORT|ER_ALARM|ER_LOCK,
                           "logic grp rslv err:read AmpAng  element flag set err\n"
                           ,0,0);
            }
            assert(FALSE);  /* 若非以上类型,则告警   */
            return  EP_SYS_ERR;
        ...
        {
            LOG_Dbg_Msg("Error, Read AmpAng  Tuyuan  Yaoce  Flag  Error!\n"
                        ,0,0,0,0,0,0);
            if(ENG_MODE == 0)
            {
                ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                           ER_REPORT|ER_ALARM|ER_LOCK,
                           "逻辑图解析错误:读取幅值相角图元的遥测设置错误\n"
                           ,0,0);
            }
            else if(ENG_MODE == 1)
            {
                ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                           ER_REPORT|ER_ALARM|ER_LOCK,
                           "Error, Read AmpAng  element  remote measurement set  Error\n"
                           ,0,0);
            }
            assert(FALSE);  /* 若非以上类型,则告警   */
            return  EP_SYS_ERR;
        ...
        {
            LOG_Dbg_Msg("Error, Read AmpAng  Tuyuan  MeasureValue  Flag  Error!\n"
                        ,0,0,0,0,0,0);
            if(ENG_MODE == 0)
            {
                ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                           ER_REPORT|ER_ALARM|ER_LOCK,
                           "逻辑图解析错误:读取幅值相角图元的测量设置错误\n"
                           ,0,0);
            }
            else if(ENG_MODE == 1)
            {
                ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                           ER_REPORT|ER_ALARM|ER_LOCK,
                           "logic grp rslv err:get AmpAng   element measurement  set err\n"
                           ,0,0);
            }
            assert(FALSE);  /* 若非以上类型,则告警   */
            return  EP_SYS_ERR;
    ...
    /* 进行节点其他未获得初值的信息,赋初值 */
    bOtherInitSuccess=RE_AmpAngTuyuanReadFileOtherInit
                      (pInitNode);
...
BOOL      RE_AmpAngTuyuanReadFileOtherInit
(AmpAng_Init_Node_Type * pElemInitNodePointer)
{
    pElemInitNodePointer->PublicElemData.elem.ucType=RE_AMPANG;
    /* 设置包裹后的初始化函数 */
    pElemInitNodePointer->PublicElemData.pfInitScanFunc=
        RE_AmpAngTuyuanScanInit;
    /* 设置获取扫描节点输出的指针的函数 */
    pElemInitNodePointer->PublicElemData.pfGetScanNodeOutIOFunc=
        RE_AmpAngTuyuanGetOutIO;
...
EP_STATUS   RE_AmpAngTuyuanScanInit(NODE * pElemInitNode,NODE  *pElemScanNode,
                                    LIST  *pGrpScanNodeList,BOOL  bPartGrpRunFlag)
{

    AmpAng_Init_Node_Type    *pInitNode;
    AmpAng_Scan_Node_Type    *pScanNode;
    NODE  *  pCurSourceNode;
    ...
    pInitNode=(AmpAng_Init_Node_Type    *)pElemInitNode->pTuyuan;
    pScanNode=(AmpAng_Scan_Node_Type    *)pElemScanNode->pTuyuan;
    ...
    for(i=0; i<1; i++)
    {
        ...
        {
            LOG_Dbg_Msg("Error,Get  AmpAng  Tuyuan  Input Source  Init  failure!\n"
                        ,0,0,0,0,0,0);
            if(ENG_MODE == 0)
            {
                ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                           ER_REPORT|ER_ALARM|ER_LOCK,
                           "逻辑图解析错误:获得幅值相角图元输入来源错误\n"
                           ,0,0);
            }
            else if(ENG_MODE == 1)
            {
                ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                           ER_REPORT|ER_ALARM|ER_LOCK,
                           "logic grp rslv err:get AmpAng  element Input source failure\n"
                           ,0,0);
            }
            assert(FALSE);  /* 若非以上类型,则告警   */
            return  EP_SYS_ERR;
        ...
        {
            LOG_Dbg_Msg("Error,Get AmpAng Tuyuan  input  info  failure!\n"
                        ,0,0,0,0,0,0);
            if(ENG_MODE == 0)
            {
                ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                           ER_REPORT|ER_ALARM|ER_LOCK,
                           "逻辑图解析错误:获得幅值相角图元输入来源失败\n"
                           ,0,0);
            }
            else if(ENG_MODE == 1)
            {
                ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                           ER_REPORT|ER_ALARM|ER_LOCK,
                           "logic grp rslv err:get AmpAng  element Input signal type err\n"
                           ,0,0);
            }
            assert(FALSE);  /* 若非以上类型,则告警   */
            return  EP_SYS_ERR;
        ...
        {
            LOG_Dbg_Msg("Error,AmpAng  Tuyuan Input  Signal Type  can't  Match!\n"
                        ,0,0,0,0,0,0);
            if(ENG_MODE == 0)
            {
                ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                           ER_REPORT|ER_ALARM|ER_LOCK,
                           "逻辑图解析错误:幅值相角图元的输入的信号类型错误\n"
                           ,0,0);
            }
            else if(ENG_MODE == 1)
            {
    ...
    for(i=0; i<2; i++)
    {
        ...
        if(bPartGrpRunFlag)
        {
            ...
            if((*pbCurLuboSetFlag))
            {
                ...
                {
                    LOG_Dbg_Msg(" Error,Set  AmpAng  Tuyuan   Output  Lubo  ID is  \'%s\'  Value  Error!\n",
                                (int)(*pstrCurLuboID), 0, 0, 0, 0, 0);
                    if(ENG_MODE == 0)
                    {
                        ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                                   ER_REPORT|ER_ALARM|ER_LOCK,
                                   "逻辑图解析错误:幅值相角图元的输出的录波标识为\'%s\'的录波设置错误\n",
                                   (int)(*pstrCurLuboID), 0);
                    }
                    else if(ENG_MODE == 1)
                    {
                        ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                                   ER_REPORT|ER_ALARM|ER_LOCK,
                                   "logic grp rslv err:AmpAng  element  output disturbance record id \'%s\'  set err\n"
                                   ,(int)(*pstrCurLuboID),0);
                    }
                    assert(FALSE);  /* 若非以上类型,则告警   */
            ...
            if((*pbCurFlagSetFlag))
            {
                ...
                if(SetCurOutAttribResult!=EP_SUCCESS)
                {
                    LOG_Dbg_Msg(" Error,Set  AmpAng   element Output  Flag  ID is  \'%s\'  Value  Error!\n"
                                ,(int)(*pstrCurFlagID),0,0,0,0,0);
                    if(ENG_MODE == 0)
                    {
                        ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                                   ER_REPORT|ER_ALARM|ER_LOCK,
                                   "逻辑图解析错误:幅值相角图元的输出的标志标识为\'%s\'的标志设置错误\n"
                                   ,(int)(*pstrCurFlagID),0);
                    }
                    else if(ENG_MODE == 1)
                    {
                        ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                                   ER_REPORT|ER_ALARM|ER_LOCK,
                                   "logic grp rslv err:AmpAng element  optput flag id \'%s\' flag set err\n"
                                   ,(int)(*pstrCurFlagID),0);
                    }
                    assert(FALSE);  /* 若非以上类型,则告警   */
                    return   SetCurOutAttribResult;
            ...
            if((*pbCurYaoceSetFlag))
            {
                ...
                if(SetCurOutAttribResult!=EP_SUCCESS)
                {
                    LOG_Dbg_Msg(" Error,Set  AmpAng   element Output  Yaoce  ID is  \'%s\'  Value  Error!\n"
                                ,(int)(*pstrCurYaoceID),0,0,0,0,0);
                    if(ENG_MODE == 0)
                    {
                        ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                                   ER_REPORT|ER_ALARM|ER_LOCK,
                                   "逻辑图解析错误:幅值相角图元的输出的遥测标识为\'%s\'的遥测设置错误\n"
                    ...
                    else if(ENG_MODE == 1)
                    {
                        ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                                   ER_REPORT|ER_ALARM|ER_LOCK,
                                   "logic grp rslv err:AmpAng  element Output remote measurement id \'%s\' remote measurement set err\n"
                                   ,(int)(*pstrCurYaoceID),0);
                    }
                    assert(FALSE);  /* 若非以上类型,则告警   */
                    return   SetCurOutAttribResult;
            ...
            if((*pbCurMeasureSetFlag))
            {
                ...
                if(SetCurOutAttribResult!=EP_SUCCESS)
                {
                    LOG_Dbg_Msg(" Error,Set  AmpAng   element Output measure  ID is  \'%s\'  Value  Error!\n"
                                ,(int)(*pstrCurMeasureID),0,0,0,0,0);
                    if(ENG_MODE == 0)
                    {
                        ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                                   ER_REPORT|ER_ALARM|ER_LOCK,
                                   "逻辑图解析错误:幅值相角图元的输出的测量标识为\'%s\'的测量设置错误\n"
                    ...
                    else if(ENG_MODE == 1)
                    {
                        ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                                   ER_REPORT|ER_ALARM|ER_LOCK,
                                   "Error,Set  AmpAng  element Output measurement  ID is  \'%s\'  Value  Error\n"
                                   ,(int)(*pstrCurMeasureID),0);
                    }
                    assert(FALSE);  /* 若非以上类型,则告警   */
                    return   SetCurOutAttribResult;
            ...
            if ((*pbCurAOSetFlag))
            {
                ...
                if (SetCurOutAttribResult != EP_SUCCESS)
                {
                    LOG_Dbg_Msg("Error, Set AmpAng Tuyuan Elem Output AO ID is \'%s\' Value Error!\n",
                                (int)(*pstrCurAOID), 0, 0, 0, 0, 0);

                    if (ENG_MODE == 0)
                    {
                        ER_Set_Err(EV_SOFTWARE_CHECK_ERR, ER_REPORT|ER_ALARM|ER_LOCK,
                                   "逻辑图解析错误:幅值相角图元输出的AO标识为\'%s\'的AO设置错误!\n",
                                   (int)(*pstrCurAOID), 0);
                    }
                    else if (ENG_MODE == 1)
                    {
                        ER_Set_Err(EV_SOFTWARE_CHECK_ERR, ER_REPORT|ER_ALARM|ER_LOCK,
                                   "logic grp  rslv err:Ampang Tuyuan element output measurement id \'%s\' set err!\n",
                                   (int)(*pstrCurAOID), 0);
                    }
...
EP_STATUS   RE_AmpAngCreateScanNodeInit(
    ...
    NODE * pElemInitNode)
{
    ...
    pScanNode=(AmpAng_Scan_Node_Type    *)malloc(sizeof(AmpAng_Scan_Node_Type));
    if(pScanNode==NULL)
    {
        LOG_Dbg_Msg("AmpAng  Tuyuan  malloc  failiure!\n",0,0,0,0,0,0);
        return  EP_BUF_ERR;
    ...
    /* 设置实际扫描图元类型 */
    pNode->ulTuyuanType=RE_AMPANG_SCAN;
...
EP_ELEM_IO *  RE_AmpAngTuyuanGetOutIO
(uint16_t   unOutNum,NODE  * pScanNode)
{

    AmpAng_Scan_Node_Type    *pTuyuanScanNode;

    pTuyuanScanNode=(AmpAng_Scan_Node_Type    *)pScanNode->pTuyuan;

    if(unOutNum>=2)
    {
        /* 若越界，则出错 */

        LOG_Dbg_Msg("Get One AmpAng  Tuyuan  OutputIO  Beyond  Bound!\n",0,0,0,0,0,0);
        assert(FALSE);
        return  NULL;
    }
...
void     RE_AmpAngTuyuanScan(int  iInterval,NODE  *pElemScanNode)
{
    AmpAng_Scan_Node_Type  *  pTuyuanNode;

    pTuyuanNode=(AmpAng_Scan_Node_Type   *)pElemScanNode->pTuyuan;

    pTuyuanNode->ioOutArr[0].now.fVal=RI_CPLX_MOD(pTuyuanNode->pInArr0->now.xVal);
    pTuyuanNode->ioOutArr[1].now.fVal=RI_CPLX_ANG(pTuyuanNode->pInArr0->now.xVal);
 ...
 *     NONE.
 */
void RE_AmpAngTuyuanScan2(NODE *pElemScanNode)
{
    AmpAng_Scan_Node_Type *pTuyuanNode;
    int iInterval;
