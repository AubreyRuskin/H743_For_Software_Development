...
/*       文件名                                           版本                 */
/*                                                                              */
/*      RE_EqualTuyuan.c                                    1.0                   */
/*                                                                              */
/*                                                                               */
/*     文件描述                                                                  */
/*                                                                              */
/*       该文件定义了保护功能模块中的等于比较元件的实现代码                        */
/*                                                                              */
/*      作者                                                                      */
/*                                                                              */
/*      张云                                                                  */
...
EP_STATUS   RE_EqualTuyuanReadFileInit(FILE  *fp,uint32_t  ulReadOffsetToBegain,
                                       TUYUAN_READ_FILE_INIT_DATA   *pTuyuanInitData,
                                       NODE ** pRtElemInitNodePointer)
{
    NODE  *pNode;
    Equal_Init_Node_Type    *pInitNode;
    BOOL   bReadSuccess;
    int  i;
    ...
    if(pNode==NULL)
    {
        LOG_Dbg_Msg("Equal  Tuyuan  malloc  failiure!\n",0,0,0,0,0,0);
        return  EP_BUF_ERR;

    }
    pInitNode=(Equal_Init_Node_Type    *)malloc(sizeof(Equal_Init_Node_Type));
    if(pInitNode==NULL)
    {
        LOG_Dbg_Msg("Equal  Tuyuan  malloc  failiure!\n",0,0,0,0,0,0);
        return  EP_BUF_ERR;
    ...
    bSignalInitSuccess=RE_EqualTuyuanInputSignalInit(
    ...
    if(pInitNode->fDeadAreaPara<0)
    {
        LOG_Dbg_Msg("Error,Equal  Tuyuan  Dead  Area  Para  can't is  Negative!\n",0,0,0,0,0,0);

        if(ENG_MODE == 0)
        {
            ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                       ER_REPORT|ER_ALARM|ER_LOCK,
                       "逻辑图解析错误:等于比较图元的死区不能为负\n",0,0);
        }
        else if(ENG_MODE == 1)
        {
            ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                       ER_REPORT|ER_ALARM|ER_LOCK,
                       "logic grp rslv err:equal  element dead area can't set negative\n",0,0);
        }
        assert(FALSE);
        return  EP_SYS_ERR;
    ...
    for(i=0; i<1; i++)
    {
        ...
        else
        {
            ...
            LOG_Dbg_Msg("Read Equal  Tuyuan  Lubo  Flag   from  Logrp  SeqFile  error!\n",0,0,0,0,0,0);
            return  EP_SYS_ERR;
        }
        ...
        else
        {
            ...
            LOG_Dbg_Msg("Read Equal  Tuyuan  FlagSet  Flag   from  Logrp  SeqFile  error!\n",0,0,0,0,0,0);
            return  EP_SYS_ERR;
        }
        ...
        {
            LOG_Dbg_Msg("Read  Equal  Tuyuan  Yaoxin  Flag   from  Logrp  SeqFile  error!\n",0,0,0,0,0,0);
            return  EP_SYS_ERR;
        }
    ...
    /* 进行节点其他未获得初值的信息,赋初值 */
    bOtherInitSuccess=RE_EqualTuyuanReadFileOtherInit
                      (pInitNode);
...
EP_STATUS   RE_EqualTuyuanScanInit(NODE * pElemInitNode,NODE  *pElemScanNode,
                                   LIST  *pGrpScanNodeList,BOOL  bPartGrpRunFlag)
{

    Equal_Init_Node_Type    *pInitNode;
    Equal_Scan_Node_Type    *pScanNode;
    NODE  *  pCurSourceNode;
    int  i;
    ...
    pInitNode=(Equal_Init_Node_Type    *)pElemInitNode->pTuyuan;
    pScanNode=(Equal_Scan_Node_Type    *)pElemScanNode->pTuyuan;
    ...
    for(i=0; i<(pInitNode->PublicElemData.elem.unInNum); i++)
    {
        ...
        {
            LOG_Dbg_Msg("Get  Equal Tuyuan  Input  Source  Init error!\n",0,0,0,0,0,0);
            assert(FALSE);
            return  EP_SYS_ERR;
        ...
        if(pInElem==NULL)
        {
            ...
            LOG_Dbg_Msg("Get  Equal  Tuyuan  input  info  error!\n",0,0,0,0,0,0);
        ...
        SetCurOutAttribResult=RE_EqualTuyuanSetInputIO
                              (i,pInElem,pElemScanNode);
        if(SetCurOutAttribResult!=EP_SUCCESS)
        {

            LOG_Dbg_Msg("Set  Equal  Tuyuan  input  info  error!\n",0,0,0,0,0,0);
            assert(FALSE);  /* 若非以上类型,则告警   */
            return  EP_SYS_ERR;
        ...
        {
            LOG_Dbg_Msg("Equal Tuyuan  One  Input  Signal  Type  is't  Match!\n",0,0,0,0,0,0);
            assert(FALSE);
            return  EP_SYS_ERR;
        }
    ...
    /* 循环设定图元的输出属性*/
    pCurElemOutput=RE_EqualTuyuanGetOutIO
                   (0,pElemScanNode);
    ...
    if(pCurElemOutput==NULL)
    {
        ...
        LOG_Dbg_Msg("Get  Equal  Tuyuan  Output  info  error!\n",0,0,0,0,0,0);
        assert(FALSE);  /* 若非以上类型,则告警   */
        return  EP_SYS_ERR;
    ...
    if(bPartGrpRunFlag)
    {
        ...
        if((*pbCurLuboSetFlag))
        {
            ...
            if(SetCurOutAttribResult!=EP_SUCCESS)
            {
                LOG_Dbg_Msg("Set  Equal  Tuyuan  \'%s\'  Lubo  Value  Error!\n"
                            ,(int)(*pstrCurLuboID),0,0,0,0,0);
                if(ENG_MODE == 0)
                {
                    ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                               ER_REPORT|ER_ALARM|ER_LOCK,
                               "逻辑图解析错误:设置等于比较图元录波错误,录波标识为\'%s\'\n"
                               ,(int)(*pstrCurLuboID),0);
                }
                else if(ENG_MODE == 1)
                {
                    ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                               ER_REPORT|ER_ALARM|ER_LOCK,
                               "logic grp rslv err:set equal  element disturbance record id \'%s\' err\n"
                               ,(int)(*pstrCurLuboID),0);
                }
                assert(FALSE);
        ...
        if((*pbCurFlagSetFlag))
        {
            ...
            if(SetCurOutAttribResult!=EP_SUCCESS)
            {
                LOG_Dbg_Msg("Set  Equal  Tuyuan  \'%s\'  Flag  Value  Error!\n"
        ...
        if((*pbCurYaoxinSetFlag))
        {
            ...
            if(SetCurOutAttribResult!=EP_SUCCESS)
            {
                LOG_Dbg_Msg("Set  Equal  Tuyuan  \'%s\'  Yaoxin  Value  Error!\n"
                ...
                else if(ENG_MODE == 1)
                {
                    ER_Set_Err(EV_SOFTWARE_CHECK_ERR,
                               ER_REPORT|ER_ALARM|ER_LOCK,
                               "logic grp rslv err:set equal  element remote signal id \'%s\' err\n"
                               ,(int)(*pstrCurYaoxinID),0);
                }
                assert(FALSE);
                return   SetCurOutAttribResult;
...
/*  在读取文件后,进行图元相关未能从文件中获得的其他信息的初始化
      ...
      返回值,成功,返回真,失败返回假
*/
BOOL      RE_EqualTuyuanReadFileOtherInit
(Equal_Init_Node_Type * pElemInitNodePointer)
{

    pElemInitNodePointer->PublicElemData.elem.ucType=RE_EQUAL;
    /* 设置包裹后的初始化函数 */
    pElemInitNodePointer->PublicElemData.pfInitScanFunc=
        RE_EqualTuyuanScanInit;

    /* 设置获得IO输出的函数 */
    pElemInitNodePointer->PublicElemData.pfGetScanNodeOutIOFunc
        =RE_EqualTuyuanGetOutIO;
 ...
 */
BOOL     RE_EqualTuyuanInputSignalInit(uint8_t  ucSignalType,
                                       Equal_Init_Node_Type    *pElemInitNodePointer)
{
    ...
    switch(ucSignalType)
    {
        ...
        default:
            assert(FALSE);  /* 若非以上类型,则告警   */
            LOG_Dbg_Msg("Error,Equal  Tuyuan  Input  Signal   Type  isn't  Expected!\n",0,0,0,0,0,0);
            return  FALSE;
            break;
    }
...
EP_STATUS   RE_EqualCreateScanNodeInit(
    ...
    NODE * pElemInitNode)
{
    ...
    pScanNode=(Equal_Scan_Node_Type    *)malloc
              (sizeof(Equal_Scan_Node_Type));
    if(pScanNode==NULL)
    {
        LOG_Dbg_Msg("Equal  Tuyuan  malloc  failiure!\n",0,0,0,0,0,0);
        return  EP_BUF_ERR;
    ...
    switch(pInitNode->ucSignalDataType)
    {
        case  0:
            ...
            pNode->ulTuyuanType=RE_FLOAT_EQUAL_SCAN;
            break;
        case  1:

            pNode->ulTuyuanType=RE_UNSIGNED_32INT_EQUAL_SCAN;

            break;
        case  2:

            pNode->ulTuyuanType=RE_SIGNED_32INT_EQUAL_SCAN;

            break;
        default:


            LOG_Dbg_Msg("Equal  Tuyuan  Signal  Data  Type  Error!\n",0,0,0,0,0,0);
            assert(FALSE);
            return  EP_SYS_ERR;
            break;
    }
    ...
    pScanNode->fDeadAreaPara=pInitNode->fDeadAreaPara;
...
EP_ELEM_IO *  RE_EqualTuyuanGetOutIO
(uint16_t   unOutNum,NODE  * pScanNode)
{

    Equal_Scan_Node_Type    *pTuyuanNode;

    pTuyuanNode=(Equal_Scan_Node_Type    *)pScanNode->pTuyuan;
    if(unOutNum>=1)
    {
        /* 若越界，则出错 */

        LOG_Dbg_Msg("Get One  Equal  Tuyuan  OutputIO  Beyond  Bound!\n",0,0,0,0,0,0);
        assert(FALSE);
        return  NULL;
...
EP_STATUS   RE_EqualTuyuanSetInputIO
 ...
 NODE  * pScanNode)
{
    ...
    switch(ucInputNum)
    {
        ...
        default:
            LOG_Dbg_Msg("Set One Equal  Tuyuan  InputIO  Beyond  Bound!\n",0,0,0,0,0,0);
            assert(FALSE);
            return  EP_SYS_ERR;
            break;
    }
