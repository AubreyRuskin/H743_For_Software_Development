...
/*       文件名                                           版本                 */
/*                                                                              */
/*      RE_ConstZeroTuyuan.c                                   1.0                   */
/*                                                                              */
/*                                                                               */
/*     文件描述                                                                  */
/*                                                                              */
/*       该文件定义了保护功能模块中的恒0元件的实现代码                        */
/*                                                                              */
/*      作者                                                                      */
/*                                                                              */
/*      张云                                                                  */
...
EP_STATUS   RE_ConstZeroTuyuanReadFileInit(FILE  *fp,uint32_t  ulReadOffsetToBegain,
        TUYUAN_READ_FILE_INIT_DATA   *pTuyuanInitData,
        NODE ** pRtElemInitNodePointer)
{
    NODE  *pNode;
    ConstZero_Init_Node_Type    *pInitNode;
    BOOL   bReadSuccess;
    int  i;
    ...
    if(pNode==NULL)
    {
        LOG_Dbg_Msg("ConstZero  Tuyuan  malloc  failiure!\n",0,0,0,0,0,0);
        return  EP_BUF_ERR;

    }
    pInitNode=(ConstZero_Init_Node_Type    *)malloc(sizeof(ConstZero_Init_Node_Type));
    if(pInitNode==NULL)
    {
        LOG_Dbg_Msg("ConstZero  Tuyuan  malloc  failiure!\n",0,0,0,0,0,0);
        return  EP_BUF_ERR;
    ...
    /* 进行节点其他未获得初值的信息,赋初值 */
    bOtherInitSuccess=RE_ConstZeroTuyuanReadFileOtherInit
                      (pInitNode);
...
EP_STATUS   RE_ConstZeroTuyuanScanInit(NODE * pElemInitNode,NODE  *pElemScanNode,
...
/*  在读取文件后,进行图元相关未能从文件中获得的其他信息的初始化
      ...
      返回值,成功,返回真,失败返回假
*/
BOOL      RE_ConstZeroTuyuanReadFileOtherInit
(ConstZero_Init_Node_Type * pElemInitNodePointer)
{

    /*  设置图元类型*/
    pElemInitNodePointer->PublicElemData.elem.ucType=RE_ZERO;
    /* 设置包裹后的初始化函数 */
    pElemInitNodePointer->PublicElemData.pfInitScanFunc=
        RE_ConstZeroTuyuanScanInit;

    /* 设置获得IO输出的函数 */
    pElemInitNodePointer->PublicElemData.pfGetScanNodeOutIOFunc
        =RE_ConstZeroTuyuanGetOutIO;
    return   TRUE;
...
EP_STATUS   RE_ConstZeroCreateScanNodeInit(
    NODE **pReturnElemScanNodePointer,
    NODE * pElemInitNode)
{
    NODE  *pNode;
    ConstZero_Scan_Node_Type    *pScanNode;
    ConstZero_Init_Node_Type    *pInitNode;

    pInitNode=(ConstZero_Init_Node_Type    *)pElemInitNode->pTuyuan;
    ...
    pScanNode=(ConstZero_Scan_Node_Type    *)malloc
              (sizeof(ConstZero_Scan_Node_Type));
    if(pScanNode==NULL)
    {
        LOG_Dbg_Msg("ConstZero  Tuyuan  malloc  failiure!\n",0,0,0,0,0,0);
        return  EP_BUF_ERR;
    }
    pNode->ulTuyuanType=RE_ZERO_SCAN;
...
EP_ELEM_IO *  RE_ConstZeroTuyuanGetOutIO
(uint16_t   unOutNum,NODE  * pScanNode)
{
    ConstZero_Scan_Node_Type    *pTuyuanNode;

    pTuyuanNode=(ConstZero_Scan_Node_Type    *)pScanNode->pTuyuan;
    if(unOutNum>=1)
    {
        /* 若越界，则出错 */

        LOG_Dbg_Msg("Get One ConstZero  Tuyuan  OutputIO  Beyond  Bound!\n",0,0,0,0,0,0);
        assert(FALSE);
        return  NULL;
