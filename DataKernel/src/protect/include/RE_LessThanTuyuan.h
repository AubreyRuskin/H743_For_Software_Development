...
/*       文件名                                           版本                 */
/*                                                                              */
/*      RE_LessThanTuyuan.h                                    1.0                   */
/*                                                                              */
/*                                                                               */
/*     文件描述                                                                  */
/*                                                                              */
/*       该头文件定义了保护功能模块中的小于比较元件的文件头                        */
/*                                                                              */
/*      作者                                                                      */
/*                                                                              */
/*      张云                                                                  */
...
#ifndef RE_LessThanTuyuan_H
#define RE_LessThanTuyuan_H
...
/******************小于比较图元的初始化和扫描时的数据节点定义**********************************************/

/*小于比较图元初始化时，保存的小于比较图元数据结构节点，
...
__inline__  static   void     RE_FloatLessThanTuyuanScan(NODE *pElemScanNode)
{

    LessThan_Scan_Node_Type    * pTuyuanNode;
    pTuyuanNode=(LessThan_Scan_Node_Type    *)pElemScanNode->pTuyuan;
    if((pTuyuanNode->pInArr0->now.fVal)>=
            (pTuyuanNode->pInArr1->now.fVal))
    {
        /*大部分情况都应该为假*/
        pTuyuanNode->ioOut.now.bVal=FALSE;

        return;
    }
    else
    {
        pTuyuanNode->ioOut.now.bVal=TRUE;
...
__inline__  static   void     RE_Unsigned32IntLessThanTuyuanScan(NODE *pElemScanNode)
{

    LessThan_Scan_Node_Type    * pTuyuanNode;
    pTuyuanNode=(LessThan_Scan_Node_Type    *)pElemScanNode->pTuyuan;
    if((pTuyuanNode->pInArr0->now.ulVal)>=
            (pTuyuanNode->pInArr1->now.ulVal))
    {
        /*大部分情况都应该为假*/
        pTuyuanNode->ioOut.now.bVal=FALSE;
        return;
...
__inline__  static   void     RE_Signed32IntLessThanTuyuanScan(NODE *pElemScanNode)
{


    LessThan_Scan_Node_Type    * pTuyuanNode;
    pTuyuanNode=(LessThan_Scan_Node_Type    *)pElemScanNode->pTuyuan;
    if((pTuyuanNode->pInArr0->now.lVal)>=
            (pTuyuanNode->pInArr1->now.lVal))
    {
        /*大部分情况都应该为假*/
        pTuyuanNode->ioOut.now.bVal=FALSE;
        return;
