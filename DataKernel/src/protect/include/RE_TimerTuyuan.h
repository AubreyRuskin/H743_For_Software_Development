...
/*       文件名                                           版本                 */
/*                                                                              */
/*      RE_TimerTuyuan.h                                    1.0                   */
/*                                                                              */
/*                                                                               */
/*     文件描述                                                                  */
/*                                                                              */
/*       该头文件定义了保护功能模块中的时间继电器元件的文件头                        */
/*                                                                              */
/*      作者                                                                      */
/*                                                                              */
/*      张云                                                                  */
...
#ifndef RE_TimerTuyuan_H
#define RE_TimerTuyuan_H
...
/******************时间继电器图元的初始化和扫描时的数据节点定义**********************************************/

/*时间继电器图元初始化时，保存的时间图元数据结构节点，
...
typedef  struct
{
    ...
    /*时间继电器的相关信息 */
    uint8_t   ucStartSourceType;/* 启动延时来源类型 0为定植,1为常数 */
    uint8_t   ucReturnSourceType;/* 返回延时来源类型 0为定植,1为常数*/

    uint32_t   ulStartSourceValue;/* 启动延时的大小,单位为微秒 */
    uint32_t   ulReturnSourceValue;/* 返回延时的大小，单位为微妙 */

    uint32_t   ulStartSourceValueAiCnt;/* 启动延时的大小,单位为采样节拍数 */
    uint32_t   ulReturnSourceValueAiCnt;/* 返回延时的大小，单位为采样节拍数 */
    ...
    /*  定义启动延时为定植时的定植逻辑标识 */
    char   strStartSettingID[MAX_LOGID_STR_LEN+1];

    /*  定义返回延时为定植时的定植逻辑标识 */
    char   strReturnsSettingID[MAX_LOGID_STR_LEN+1];



}  Timer_Init_Node_Type;
...
typedef  struct
{
    ...
    */
    ...
    EP_ELEM_IO *   pInArr0;/*时间图元输入数据指针0  */
    /* 输出量数组，存放本图元输出结果 */
    EP_ELEM_IO   ioOut;

    BOOL   bStartDelayFlag;/* 动作延时标志 */
    BOOL   bBackDelayFlag;/* 返回延时标志 */

    uint32_t   ulStartDelayCounter;/* 动作延时启动后的计时器，已微秒为单位  */
    uint32_t   ulBackDelayCounter;/*  返回延时启动后的计时器，以微妙为单位*/

    uint32_t   ulStartSourceValue;/* 启动延时的大小,单位为微秒 */
    uint32_t   ulReturnSourceValue;/* 返回延时的大小，单位为微妙 */

    uint32_t   ulStartDelayAiCnt;/* 动作延时启动后的计时器，以采样节拍数为单位  */
    uint32_t   ulBackDelayAiCnt;/*  返回延时启动后的计时器，以采样节拍数为单位*/
    ...
    SCI_SETTING_INFO_TYPE   StartSettingSourceInfo;/* 若启动延时类型为
                        定植时的定植信息 */


    SCI_SETTING_INFO_TYPE   ReturnSettingSourceInfo;/* 若返回延时类型为
                        定植时的定植信息 */


}  Timer_Scan_Node_Type;
