...
/*       文件名                                           版本                 */
/*                                                                              */
/*      RE_EventTuyuan.h                                    1.0                   */
/*                                                                              */
/*                                                                               */
/*     文件描述                                                                  */
/*                                                                              */
/*       该头文件定义了保护功能模块中的事件元件的文件头                        */
/*                                                                              */
/*      作者                                                                      */
/*                                                                              */
/*      张云                                                                  */
...
#ifndef RE_EventTuyuan_H
#define RE_EventTuyuan_H
...
typedef  struct
{
    ...
    EP_ELEM_IO *   apInParaArr[MAX_EVENT_INPUT_COUNT-1];/*事件图元参数输入数据指针数组
                                                         事件触发信号不在该数组里面*/


    /* 事件图元的输入信号类型数组*/
    uint8_t   aucSourceSignalTypeArr[MAX_EVENT_INPUT_COUNT];

    /* 事件图元的输入源顺序号数组*/
    uint16_t   aunInSourceSeqNoArr[MAX_EVENT_INPUT_COUNT];

    /*  事件图元的输入源在相应图元的输出号*/
    uint8_t    aucInSourceOutputNumArr[MAX_EVENT_INPUT_COUNT];


    /* 事件逻辑标识  */
    char   strEventID[MAX_LOGID_STR_LEN+1];

}  Event_Init_Node_Type;
...
typedef  struct
{
    ...
    EP_ELEM_IO *   pInArr0;/*事件触发信号的输入数据指针  */


    int16_t   nEventNum;/*由图元初始化时得到的事件的序号,供触发事件时使用  */

    EP_CHART_MSG  *pChartMsg;  /*逻辑图信息*/
    BOOL bFstEnter;		/* 初次进入 */

}  Event_Scan_Node_Type;
 ...
 *     NONE.
 */
extern void RE_MultiEventTuyuanScan(PARTGRP_ATTRIB_TYPE *PartGrpAttrib);
