...
/*       文件名                                           版本                 */
/*                                                                              */
/*      RE_AISetTuyuan.h                                    1.0                   */
/*                                                                              */
/*                                                                               */
/*     文件描述                                                                  */
/*                                                                              */
/*       该头文件定义了保护功能模块中的AI输入集元件的文件头                        */
/*                                                                              */
/*      作者                                                                      */
/*                                                                              */
/*      张云                                                                  */
...
#ifndef RE_AISetTuyuan_H
#define RE_AISetTuyuan_H
...
typedef  struct
{
    ...
    uint8_t     ucSignalValueType;/*  信号值的属性， */

    /*  定义AI输入集的输入源的逻辑标识数组*/
    char   strInputSourceIDArr[MAX_AISET_OUTPUT_COUNT][MAX_LOGID_STR_LEN+1];
    /* 输入源为AI时的通道距第0号通道的偏移量 数组 */
    unsigned long  AISourceChOffsetArr[MAX_AISET_OUTPUT_COUNT];
    uint8_t    ucOutputCnt;

}  AISet_Init_Node_Type;
...
typedef  struct
{
    ...
    */
    ...
    uint8_t    ucOutputCnt_8;     /*输出个数/8  */
    uint8_t    ucOutputCnt_m;      /*输出个数/8后的余数  */

    /* 输出量数组，存放本图元输出结果 */
    EP_ELEM_IO   ioOutArr[MAX_AISET_OUTPUT_COUNT];
    /* AI集中后的输出IO指针数组,若某成员为空,则表示未集中,2011-7-27  ZY  */
    EP_ELEM_IO   *apCollectOutArr[MAX_AISET_OUTPUT_COUNT];
    /* 输入源为AI时的通道距第0号通道的偏移量数组  */
    unsigned long  AISourceChOffsetArr[MAX_AISET_OUTPUT_COUNT];

}  AISet_Scan_Node_Type;
...
extern void     RE_FloatAISetTuyuanScan(NODE *pElemScanNode);
...
extern void     RE_ComplexAISetTuyuanScan(NODE *pElemScanNode);
