...
/*       文件名                                           版本                 */
/*                                                                              */
/*      RE_DISetTuyuan.h                                    1.0                   */
/*                                                                              */
/*                                                                               */
/*     文件描述                                                                  */
/*                                                                              */
/*       该头文件定义了保护功能模块中的开入集元件的文件头                        */
/*                                                                              */
/*      作者                                                                      */
/*                                                                              */
/*      张云                                                                  */
...
#ifndef RE_DISetTuyuan_H
#define RE_DISetTuyuan_H
...
typedef  struct
{
    ...
    /*  定义开入集的输入源的逻辑标识数组*/
    char   strInputSourceIDArr[MAX_DISET_OUTPUT_COUNT][MAX_LOGID_STR_LEN+1];
    uint8_t    ucOutputCnt;     /*输出个数  */

    /* 输入源存储地址偏移量 */
    uint32_t DISourceChOffsetArr[MAX_DISET_OUTPUT_COUNT];
}  DISet_Init_Node_Type;
...
typedef  struct
{
    ...
    */
    ...
    uint8_t   ucOutputCnt_8;  /* 输出个数有几个8的倍数 */
    uint8_t   ucOutputCnt_m;   /*输出个数用8除后的余数 */

    /* 输出量数组，存放本图元输出结果 */
    EP_ELEM_IO   ioOutArr[MAX_DISET_OUTPUT_COUNT];

    /* 输入源存储地址偏移量 */
    uint32_t DISourceChOffsetArr[MAX_DISET_OUTPUT_COUNT];

    /* DI集中后的输出IO指针数组,若某成员为空,则表示未集中,2011-7-27  ZY  */
    EP_ELEM_IO   *apCollectOutArr[MAX_DISET_OUTPUT_COUNT];
}  DISet_Scan_Node_Type;
...
extern void     RE_DISetTuyuanScan(NODE *pElemScanNode);
