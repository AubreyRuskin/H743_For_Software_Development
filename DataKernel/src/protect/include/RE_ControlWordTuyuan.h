...
/*       文件名                                           版本                 */
/*                                                                              */
/*      RE_ControlWordTuyuan.h                                    1.0                   */
/*                                                                              */
/*                                                                               */
/*     文件描述                                                                  */
/*                                                                              */
/*       该头文件定义了保护功能模块中的控制字元件的文件头                        */
/*                                                                              */
/*      作者                                                                      */
/*                                                                              */
/*      张云                                                                  */
...
#ifndef RE_ControlWordTuyuan_H
#define RE_ControlWordTuyuan_H
...
/******************控制字图元的初始化和扫描时的数据节点定义**********************************************/

/*控制字图元初始化时，保存的控制字图元数据结构节点，
...
typedef  struct
{
    ...
    /* 控制字图元的输入信号类型数组*/
    uint8_t   aucSourceSignalTypeArr[MAX_CONTROLWORD_INPUT_COUNT];

    /* 控制字图元的输入源顺序号数组*/
    uint16_t   aunInSourceSeqNoArr[MAX_CONTROLWORD_INPUT_COUNT];

    /*  控制字图元的输入源在相应图元的输出号*/
    uint8_t    aucInSourceOutputNumArr[MAX_CONTROLWORD_INPUT_COUNT];
    ...
    /*  定义控制字的逻辑标识 */
    char   strControlWordID[MAX_LOGID_STR_LEN+1];

}  ControlWord_Init_Node_Type;
...
typedef  struct
{
    ...
    */
    ...
    uint32_t   ulControlWordCurValue;/*控制字当前值,单位为控制字  */
    EP_ELEM_IO   ioOut;
    EP_ELEM_IO *   apInArr[MAX_CONTROLWORD_INPUT_COUNT];/*控制字输入数据指针数组   */

    SCI_SETTING_INFO_TYPE   ControlWordInfo;/* 控制字的定植信息*/

    uint16_t unInNum;                   /* 总的输入个数 */

}  ControlWord_Scan_Node_Type;
