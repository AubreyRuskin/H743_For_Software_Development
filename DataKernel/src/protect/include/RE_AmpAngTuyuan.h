...
/*       文件名                                           版本                 */
/*                                                                              */
/*      RE_AmpAngTuyuan.h                                    1.0                   */
/*                                                                              */
/*                                                                               */
/*     文件描述                                                                  */
/*                                                                              */
/*       该头文件定义了保护功能模块中的幅值相角元件的文件头                        */
/*                                                                              */
/*      作者                                                                      */
/*                                                                              */
/*      张云                                                                  */
...
#ifndef RE_AmpAngTuyuan_H
#define RE_AmpAngTuyuan_H
...
/******************幅值相角图元的初始化和扫描时的数据节点定义**********************************************/

/*幅值相角图元初始化时，保存的图元数据结构节点，
...
typedef  struct
{
    ...
    /*定义输出的录波逻辑标识   */
    char   aStrLuboIDArr[2][MAX_LOGID_STR_LEN+1];

    /*定义输出的标志逻辑标识   */
    char   aStrFlagIDArr[2][MAX_LOGID_STR_LEN+1];

    /*定义输出的遥测逻辑标识   */
    char   aStrYaoceIDArr[2][MAX_LOGID_STR_LEN+1];

    /* AO logic symbol. */
    char aStrAOChIDArr[2][MAX_LOGID_STR_LEN+1];

    /*定义输出的测量逻辑标识*/
    char   aStrMeasureIDArr[2][MAX_LOGID_STR_LEN+1];

}  AmpAng_Init_Node_Type;
...
typedef  struct
{
    ...
    */
    ...
    EP_ELEM_IO *   pInArr0;/*输入数据指针数组*/
    /* 输出量数组，存放本图元输出结果 */
    EP_ELEM_IO   ioOutArr[2];

}  AmpAng_Scan_Node_Type;
...
extern void     RE_AmpAngTuyuanScan(int  iInterval,NODE  *pElemScanNode);
 ...
 *     NONE.
 */
extern void RE_AmpAngTuyuanScan2(NODE *pElemScanNode);
