...
#define ENCODE_UNKNOWN		-1
#define ENCODE_UTF8	0		/* UTF-8 */
#define ENCODE_UTF16BE	1		/* UTF-16 Big-Endian */
#define ENCODE_UTF16LE	2		/* UTF-16 Little-Endian */
#define ENCODE_GB2312	3		/* GB2312 */
 ...
 * Version number...
 ...
 2010-03-29	v3.0.0	修改字符读取函数 mxml_add_char 和 mxml_file_getc，不在区分字符编码，读取字符时均按
 					ascii 码处理，中文按16进制数字处理。可支持UTF-8和GB2312文件编码。
*/
...
extern void					mxmlDelete(mxml_node_t *node);
extern const char			*mxmlElementGetAttr(mxml_node_t *node, const char *name);
const char			*mxmlElementGetValue(mxml_node_t *node);
extern mxml_node_t		*mxmlFindElement(mxml_node_t *node, mxml_node_t *top,
        const char *name, const char *attr,
        const char *value, int descend);

extern mxml_node_t 		*mxmlNextElement(mxml_node_t *node,mxml_node_t *top,int descend);

extern void					mxmlNodePrint(mxml_node_t *node);
